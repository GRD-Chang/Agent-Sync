...
- 校验 role/action/target 组合；实现类 run 只能由 Evaluator `pass -> system` 完成。
...
- Generator `stop -> system` 和 Generator/Evaluator `ask_user -> user` 被拒绝或进入 repair。
...
- 支持 route：
  ...
  - `generator -> ready_for_review(evaluator)`
  - `evaluator continue build round -> generator`
...
- fake generator 输出 `ready_for_review -> evaluator` 后 state 进入 `evaluating_final`。
- fake evaluator `continue -> generator` 后 state 回到 `generating`。
...
## 10. M8：Repair 与 failed run resume

### Goal

当 agent 最终 envelope、固定 artifact 或 route 语义不符合协议时，允许 resume 同 session 修复 1-2 轮，再决定是否 failed。当 run 因可重试 runner 级错误进入 `failed` 时，允许用户通过 `task-bridge codex-team resume <run_id>` 恢复。

### Scope

- 增加 `resume_policy = repair_protocol_errors`。
- 支持 `resume_repair_attempts = 2`。
- 增加 `codex-team resume`：仅允许作用于 `failed` run，且只恢复 runner/auth/timeout/lock/missing-envelope/max-steps 等可重试错误。
- 恢复时优先从 `last_error.details.thread_id/session_id/stdout_log/stdout_tail` 提取 thread id，并调用 `codex exec resume`；没有 thread id 时重跑当前 owner。
- `resume --max-steps 0` 不启动 runner；`resume --max-steps 1` 最多运行一个 agent step。
- 构造 repair prompt，明确：
...
- 不支持跨角色、跨 build round、跨 repo resume。
...
- failed run 可以通过 `resume` 恢复到失败前 owner 对应 state；不可恢复错误会被拒绝。
...
- Prompt 共享政策从 `ARTIFACT_DIRECTORY_SECTION`、`HANDOFF_ECONOMY_POLICY`、`BLOCKING_FIX_POLICY` 生成，避免同一政策散落成多套措辞。
...
- Planner prompt 明确 `required_scope` 不应是小 ticket 列表，并用 `product_spec` 和 `engineering_guidance` 定义完整 build round 的交付边界与高层工作区域。
- Markdown artifact 模板只保留少量关键锚点，不把 `.md` 变成 schema；细节通过自然语言表达。
- `plan.md` 关键锚点为 `goal_and_scope`、`product_spec`、`architecture_direction`、`engineering_guidance`、`acceptance_and_verification`、`risks_and_open_questions`。
- `implementation.md` 关键锚点为 `summary`、`build_round_status`、`changes_and_evidence`、`scope_status`、`feedback_addressed`、`known_limitations`。
- `evaluation.md` 关键锚点为 `summary`、`build_review`、`blocking_fixes`、`non_blocking_findings`、`scope_status`、`route_decision`。
- Generator prompt 明确在 plan 边界内连续推进完整 build round，直到完整 spec 已实现、自测完成并可进入 Evaluator review。
- Generator prompt 明确不是机械地尽量完成多个 phase，也避免每个小步骤都交给 Evaluator。
- Generator prompt 明确能自行推进的问题应继续实现和自测，不把 Evaluator 当作每个小步骤后的确认按钮。
- Generator prompt 明确只有完整 spec 已实现、自测完成且 `implementation.md` 已写好时，才使用 `action=ready_for_review,target=evaluator`；helper、schema、fixture、CLI 子命令、局部 bug fix、单个测试通过和半成品都不能触发 `ready_for_review`。
- Generator prompt 明确 latest evaluation 为 `continue` 时，必须读取 `build_review`、`non_blocking_findings`、`scope_status` 和 `route_decision` 作为下一批输入。
...
- Evaluator prompt 明确自己是 round-level QA，不是频繁打断 Generator 的调度器；final 前必须检查 `final_condition` 和 `remaining_scope`。
- Evaluator prompt 明确 `blocking_fixes` / `required_fixes` 只放阻塞项，非阻塞问题写入 `non_blocking_findings` 或 `route_decision` 的下一批建议。
- Evaluator prompt 明确默认路由：blocking issue 或完整 spec 未满足 -> `needs_fix`；当前 build round 可接受且剩余工作仅为 later scope / 非阻塞增强 / 可接受残余风险 -> `continue`；final condition met 且无 remaining scope -> `pass`；边界无法判定 -> `needs_design`。
...
每个 implementation milestone 的 Evaluator 都必须检查：
...
若某个 implementation milestone 修改 `src/task_bridge/cli.py`、`pyproject.toml` 或 package data，必须额外回归：
...
若某个 implementation milestone 修改 runner 或 dispatcher，必须额外回归：
...
- `task-bridge codex-team start/status/show/logs/answer/resume/cancel` 可用。
...
- Generator 的 `ready_for_review` 不能绕过 Evaluator。
