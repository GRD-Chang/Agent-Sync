...
- 自动化 Follow-up：任务完结后如果 Leader 迟迟不布置新任务（默认 5 分钟内，可通过 `task-bridge daemon --leader-followup-seconds` 调整），Bridge 会主动提醒 Leader 确认下一步，防止自动化流水线停转。
...
- `--leader-followup-seconds 300`: 终态通知发出后，如果同一 job 仍然没有新 task，Bridge 在多久后催 Leader 决定下一步（默认 5 分钟；`0` 表示下一轮轮询即可 follow-up）。
