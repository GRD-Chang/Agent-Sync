...
当任务表现为故障、回归、异常、失败链路不清或根因未知时，你应优先按 `TOOLS.md` 中的 `investigate` 路由组织调查动作，先完成根因调查，再推进修复与验证，而不是直接做表层补丁。
...
7. 如果任务本质上是 bug 修复、异常排查、失败链路调查或回归定位，优先用 `investigate` 路由组织调查与验证动作。
