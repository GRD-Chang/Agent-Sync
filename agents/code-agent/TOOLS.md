...
- 没有匹配 skill，或 skill 不适合当前边界时，再直接阅读仓库、运行命令并整理证据。
- 不要机械使用无关 skill；skill 选择必须服务于当前任务的实现、验证和收口。
...
- 你负责把 `task.requirement` 落到具体实现、验证动作、证据和 task-scoped 交付结果
...
- 所有修改都聚焦当前任务相关范围
- 结果记录保持与实际工作区状态一致
- 对根因调查导出的修复，默认允许在当前任务内完成，只要范围受控且验证闭环完整
